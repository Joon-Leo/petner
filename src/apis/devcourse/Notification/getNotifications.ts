import {
  ...
  ExtractNotificationFromJson,
} from '@/types/devcourse/devNotification';
...
export async function getNotifications(): Promise<GetNotificationsResponse> {
  const response = await devAPI.get<devNotification[]>(
    import.meta.env.VITE_API_BASE_URL + NOTIFICATION.getNotification,
  );
